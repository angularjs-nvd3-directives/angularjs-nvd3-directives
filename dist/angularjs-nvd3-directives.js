...
angular.module('nvd3ChartDirectives', []).constant('nvd3Helpers', {
  ...
  checkElementID: function (scope, attrs, element, chart, data) {
    ...
    d3.select(svgElem).attr('viewBox', '0 0 ' + scope.width + ' ' + scope.height).datum(data).transition().duration(attrs.transitionduration === undefined ? 250 : +attrs.transitionduration).call(chart);
  },
...
;
angular.module('nvd3ChartDirectives').directive('nvd3MultiBarChart', [
  '$filter',
  'nvd3Helpers',
  function ($filter, nvd3Helpers) {
    return {
